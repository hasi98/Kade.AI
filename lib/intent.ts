...
export function hasComplexShoppingSignal(message: string): boolean {
  ...
  return (
    ...
    /\b(amma|ammi|mother|mom|mum|dad|thaththa|girlfriend|gf|boyfriend|bf|wife|husband|crush)\b/.test(lower) ||
...
export function hasEnoughInfoToSearch(message: string, history: Content[] = []): boolean {
  ...
  const concreteProduct =
    /\b(chocolate cakes?|cake|cakes|bento|ribbon cake|cupcakes?|brownies?|chocolates?|ferrero|truffle|java chocolate|biscuits?|cookies?|crackers?|gourmet hamper|food hamper|snack hamper|tea|basilur|dilmah|qualitea|coffee|spa ceylon|perfume|handbag|jewellery|jewelry|flowers?|roses?|bouquet|baby hamper|gift set|gift box)\b/i.test(lower);
  const directAction =
  ...
  const concreteAnswer =
    /\b(she|he|they|my gf|my girlfriend|my wife|my husband|my amma|my friend)\b.*\b(likes|loves|wants|is into|prefers)\b.*\b(chocolate cakes?|cakes?|chocolates?|biscuits?|cookies?|tea|coffee|flowers?|roses?|spa ceylon|perfume|jewellery|jewelry)\b/i.test(lower);
